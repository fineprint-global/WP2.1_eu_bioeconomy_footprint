##########################################################################
##  Aggregate Agricultural Land Area Imports & Exports per Major Region
##########################################################################
...
# crops cropland
#-------------------------------------------------------------------------
load(file=paste0(path,"results/crops_cropland_list.RData"))
...
# lvst cropland
#-------------------------------------------------------------------------
load(file=paste0(path,"results/livestock_cropland_list.RData"))
...
# lvst grassland
#-------------------------------------------------------------------------
load(file=paste0(path,"results/livestock_pastures_list.RData"))
all <- all[!all$com==26 & !all$value==0,]
all <- merge(all, aggreg[,c(1,3)], by.x="from", by.y="MRIO_No")
all <- aggregate(value ~ MREG_No + to + com + year + type, all, sum)
names(all)[1] <- "from"
...
write.table(file=paste0(path,"results/IM-EX_land_MREG.csv"), x = all, sep=";", dec=".")
