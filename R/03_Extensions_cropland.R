...
nrsua = 17    # number of sua commodities in file CropDb_v4_25May2015_final.xlsx (excl. total)
...
data <- read.xlsx(paste0(path,"IIASA_data/Cropdb_v4_28Jun2015_final.xlsx"), sheet = 5, startRow = 1, colNames = TRUE, rowNames = FALSE)
# Filter data with unit=3 (1000 ha)
data <- data[data$UNIT==3,c(1:4,14)]
dataset <- list()
for(year in years){
  temp <- matrix(0,nrsua+1,nrreg)
...
for(year in years){
  ...
  # Save results
  ##########################################################################
  save(extensions,extensions_abs,FDextensions, file = paste0(path,year,"/",year,"_extensions_cropland.RData"))
  
  # write.xlsx(extensions,paste0(path,year,"/",year,"_extensions_cropland.xlsx"))
  # write.xlsx(FDextensions,paste0(path,year,"/",year,"_FDextensions_cropland.xlsx"))
