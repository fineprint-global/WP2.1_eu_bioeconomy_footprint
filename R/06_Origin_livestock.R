...
setwd(path)
land.type <- c("livestock_cropland","livestock_pastures","livestock_equ.pastures")
library("openxlsx")
library(reshape)
...
rawdata <- read.xlsx(paste0(path,"IIASA_data/OthUseR_v4_28Jun2015_final.xlsx"), sheet = 5, startRow = 1, colNames = TRUE, rowNames = FALSE)
...
for(i in 1:3){
  ...
  shares.unit <- shares[shares$unit==i+2,]
  all_origin <- merge(shares.unit, all, by = c("from","year","com"))
  all_origin[,5:25] <- all_origin[,5:25] * all_origin$value
  sum(all$value)/2 == sum(all_origin$value)
